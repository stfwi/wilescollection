...
public class JEIPlugin implements mezz.jei.api.IModPlugin
{
  ...
  { return new ResourceLocation(ModWilesCollection.MODID, "jei_plugin_uid"); }
  ...
  public void registerRecipeTransferHandlers(IRecipeTransferRegistration registration)
  {
    if(!ModConfig.isOptedOut(ModContent.CRAFTING_TABLE)) {
      try {
        ...
        ModWilesCollection.logger().warn("Exception in JEI crafting table handler registration: '" + e.getMessage() + "'.");
      }
    }
  }
  ...
  public void onRuntimeAvailable(IJeiRuntime jeiRuntime)
  {
    ...
    if(!blacklisted.isEmpty()) {
      ...
      try {
        ...
        ModWilesCollection.logger().warn("Exception in JEI opt-out processing: '" + e.getMessage() + "', skipping further JEI optout processing.");
      }
    }
  }
