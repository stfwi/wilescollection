...
public class Trinkets
{
  ...
  public static class TrinketItem extends ModItem
  {
    ...
    {
      if((!stack.hasTag()) || ((world.getGameTime() & 0x3) != 0) || (!(entity instanceof ServerPlayerEntity))) return;
      final CompoundNBT tag = getNbt(stack);
      boolean changed = false;
      changed |= repairTick(stack, world, (ServerPlayerEntity)entity, itemSlot, isSelected);
    ...
    private CompoundNBT getNbt(ItemStack trinket)
    { return trinket.getOrCreateTagElement("trinket"); }

    private TrinketItem setNbt(ItemStack trinket, CompoundNBT nbt)
    ...
    { getNbt(trinket).putFloat("durability", MathHelper.clamp(value, 0f, 1f)); return this; }
    ...
    private void rechargeTick(final ItemStack trinket, final World world, final ItemEntity entity)
    {
      // Recharging by tossing items
      if(!entity.isOnGround()) {
        Vector3d pos = Vector3d.atLowerCornerOf(entity.blockPosition()).add(0.5,0, 0.5);
      ...
      // Recharging by tossing items
      {
        final List<ItemEntity> near_items = world.getEntitiesOfClass(ItemEntity.class, (new AxisAlignedBB(entity.blockPosition())).deflate(0.5).inflate(0,0.5,0), e->(e.isAlive()));
        if(!near_items.isEmpty()) {
          ...
          for(ItemEntity e:near_items) {
            ...
            if(charging_items_.containsKey(item)) {
              ...
              if(charge < max_charge) {
                ...
                final int n = MathHelper.clamp((int)Math.ceil(((float)max_charge-charge)/item_charges), 1, stack.getCount());
          ...
          if(changed) {
            ...
            world.playSound(null, entity.blockPosition(), SoundEvents.CHICKEN_EGG, SoundCategory.BLOCKS, 0.7f, 1.8f);
          }
        }
      }
      // Pickup only when player close to the item.
      {
        ...
        AxisAlignedBB bb = new AxisAlignedBB(pos.add(0.5, 2, 0.5), pos.subtract(0.5, 1, 0.5));
        if(world.getEntitiesOfClass(PlayerEntity.class, bb, e->(e.isAlive())).isEmpty()) {
    ...
    private boolean repairTick(ItemStack trinket, World world, ServerPlayerEntity player, int itemSlot, boolean isSelected)
    {
      ...
      if(repair_left <= 0) {
        world.playSound(null, player.blockPosition(), SoundEvents.CHAIN_BREAK,  SoundCategory.PLAYERS,1f, 1.5f);
      }
      return repair_left != initial_repair_left;
    }

    private int repair(ItemStack trinket, World world, ServerPlayerEntity player, int repair_left, ItemStack item)
    {
      if((repair_left<=0) || item.isEmpty() || (!item.isDamaged()) || (!item.isDamageableItem())) return repair_left;
      if(!(item.getItem() instanceof ArmorItem) && !(item.getItem() instanceof ToolItem) && !(item.getItem() instanceof ShootableItem)) return repair_left;
      final int max_repair = MathHelper.clamp(Math.min(repair_max_tick_damage_, item.getMaxDamage()/40), 1, repair_left);
