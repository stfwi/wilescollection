...
public class StandardBlocks
{
  ...
  public static class AxisAligned extends Cutout implements IStandardBlock
  {
    ...
    public AxisAligned(long config, AbstractBlock.Properties properties, final Supplier<ArrayList<VoxelShape>> shape_supplier)
    {
      super(config, properties);
    ...
    public AxisAligned(long config, AbstractBlock.Properties properties, final AxisAlignedBB[] unrotatedAABBs)
    {
      this(config, properties, ()->{
        return new ArrayList<VoxelShape>(Arrays.asList(
    ...
    public AxisAligned(long config, AbstractBlock.Properties properties, final AxisAlignedBB unrotatedAABB)
    { this(config, properties, new AxisAlignedBB[]{unrotatedAABB}); }
  ...
  public static class AxisAlignedWaterLoggable extends AxisAligned implements IWaterLoggable, IStandardBlock
  {
    public AxisAlignedWaterLoggable(long config, AbstractBlock.Properties properties, AxisAlignedBB aabb)
    { super(config|CFG_WATERLOGGABLE, properties, aabb); }

    public AxisAlignedWaterLoggable(long config, AbstractBlock.Properties properties, AxisAlignedBB[] aabbs)
    { super(config|CFG_WATERLOGGABLE, properties, aabbs); }

    public AxisAlignedWaterLoggable(long config, AbstractBlock.Properties properties, final Supplier<ArrayList<VoxelShape>> shape_supplier)
    { super(config|CFG_WATERLOGGABLE, properties, shape_supplier); }
      ...
      for(BlockState state:getStateDefinition().getPossibleStates()) {
        {
          ...
          if(state.getValue(NORTH)) shape = VoxelShapes.joinUnoptimized(shape,Auxiliaries.getUnionShape(Auxiliaries.getRotatedAABB(side_aabb, Direction.NORTH, true)), IBooleanFunction.OR);
          if(state.getValue(EAST))  shape = VoxelShapes.joinUnoptimized(shape,Auxiliaries.getUnionShape(Auxiliaries.getRotatedAABB(side_aabb, Direction.EAST, true)), IBooleanFunction.OR);
          if(state.getValue(SOUTH)) shape = VoxelShapes.joinUnoptimized(shape,Auxiliaries.getUnionShape(Auxiliaries.getRotatedAABB(side_aabb, Direction.SOUTH, true)), IBooleanFunction.OR);
          if(state.getValue(WEST))  shape = VoxelShapes.joinUnoptimized(shape,Auxiliaries.getUnionShape(Auxiliaries.getRotatedAABB(side_aabb, Direction.WEST, true)), IBooleanFunction.OR);
        ...
        }
        {
          ...
          VoxelShape shape = ((base_aabb.getXsize()==0) || (base_aabb.getYsize()==0) || (base_aabb.getZsize()==0)) ? VoxelShapes.empty() : VoxelShapes.create(base_aabb);
          if(state.getValue(NORTH)) shape = VoxelShapes.joinUnoptimized(shape,Auxiliaries.getUnionShape(Auxiliaries.getMappedAABB(Auxiliaries.getRotatedAABB(side_aabb,
                  Direction.NORTH, true), bb->bb.expandTowards(0, railing_height_extension, 0))), IBooleanFunction.OR);
          if(state.getValue(EAST))  shape = VoxelShapes.joinUnoptimized(shape,Auxiliaries.getUnionShape(Auxiliaries.getMappedAABB(Auxiliaries.getRotatedAABB(side_aabb,
                  Direction.EAST, true), bb->bb.expandTowards(0, railing_height_extension, 0))), IBooleanFunction.OR);
          if(state.getValue(SOUTH)) shape = VoxelShapes.joinUnoptimized(shape,Auxiliaries.getUnionShape(Auxiliaries.getMappedAABB(Auxiliaries.getRotatedAABB(side_aabb,
                  Direction.SOUTH, true), bb->bb.expandTowards(0, railing_height_extension, 0))), IBooleanFunction.OR);
          if(state.getValue(WEST))  shape = VoxelShapes.joinUnoptimized(shape,Auxiliaries.getUnionShape(Auxiliaries.getMappedAABB(Auxiliaries.getRotatedAABB(side_aabb,
                  Direction.WEST, true), bb->bb.expandTowards(0, railing_height_extension, 0))), IBooleanFunction.OR);
          if(shape.isEmpty()) shape = VoxelShapes.block();
  ...
  public static class HorizontalFaceAttachedWaterLoggable extends HorizontalWaterLoggable implements IWaterLoggable, IStandardBlock
  {
    ...
    public HorizontalFaceAttachedWaterLoggable(long config, AbstractBlock.Properties properties, AxisAlignedBB aabb)
    { this(config, properties, new AxisAlignedBB[]{aabb}); }

    public HorizontalFaceAttachedWaterLoggable(long config, AbstractBlock.Properties properties, AxisAlignedBB[] aabbs)
    {
      super(config|CFG_WATERLOGGABLE|CFG_LOOK_PLACEMENT, properties, new AxisAlignedBB[]{});
      registerDefaultState(super.defaultBlockState().setValue(FACE, AttachFace.WALL));
    ...
    @Override
    protected void createBlockStateDefinition(StateContainer.Builder<Block, BlockState> builder)
    { super.createBlockStateDefinition(builder); builder.add(FACE); }
    ...
    public BlockState getStateForPlacement(BlockItemUseContext context)
    {
      ...
      for(Direction direction : context.getNearestLookingDirections()) {
        final BlockState atface_state = (direction.getAxis() == Direction.Axis.Y)
          ? state.setValue(FACE, (direction == Direction.UP) ? (AttachFace.CEILING) : (AttachFace.FLOOR)).setValue(HORIZONTAL_FACING, context.getHorizontalDirection())
          : state.setValue(FACE, AttachFace.WALL).setValue(HORIZONTAL_FACING, direction.getOpposite());
        if(atface_state.canSurvive(context.getLevel(), context.getClickedPos())) return atface_state;
      }
      return null;
    }
  }
