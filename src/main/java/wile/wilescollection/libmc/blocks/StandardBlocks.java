      ...
      for(BlockState state:getStateContainer().getValidStates()) {
        {
          ...
          if(state.get(NORTH)) shape = VoxelShapes.combine(shape,Auxiliaries.getUnionShape(Auxiliaries.getRotatedAABB(side_aabb, Direction.NORTH, true)), IBooleanFunction.OR);
          if(state.get(EAST))  shape = VoxelShapes.combine(shape,Auxiliaries.getUnionShape(Auxiliaries.getRotatedAABB(side_aabb, Direction.EAST, true)), IBooleanFunction.OR);
          if(state.get(SOUTH)) shape = VoxelShapes.combine(shape,Auxiliaries.getUnionShape(Auxiliaries.getRotatedAABB(side_aabb, Direction.SOUTH, true)), IBooleanFunction.OR);
          if(state.get(WEST))  shape = VoxelShapes.combine(shape,Auxiliaries.getUnionShape(Auxiliaries.getRotatedAABB(side_aabb, Direction.WEST, true)), IBooleanFunction.OR);
        ...
        }
        {
          ...
          VoxelShape shape = ((base_aabb.getXSize()==0) || (base_aabb.getYSize()==0) || (base_aabb.getZSize()==0)) ? VoxelShapes.empty() : VoxelShapes.create(base_aabb);
          if(state.get(NORTH)) shape = VoxelShapes.combine(shape,Auxiliaries.getUnionShape(Auxiliaries.getMappedAABB(Auxiliaries.getRotatedAABB(side_aabb,
                  Direction.NORTH, true), bb->bb.expand(0, railing_height_extension, 0))), IBooleanFunction.OR);
          if(state.get(EAST))  shape = VoxelShapes.combine(shape,Auxiliaries.getUnionShape(Auxiliaries.getMappedAABB(Auxiliaries.getRotatedAABB(side_aabb,
                  Direction.EAST, true), bb->bb.expand(0, railing_height_extension, 0))), IBooleanFunction.OR);
          if(state.get(SOUTH)) shape = VoxelShapes.combine(shape,Auxiliaries.getUnionShape(Auxiliaries.getMappedAABB(Auxiliaries.getRotatedAABB(side_aabb,
                  Direction.SOUTH, true), bb->bb.expand(0, railing_height_extension, 0))), IBooleanFunction.OR);
          if(state.get(WEST))  shape = VoxelShapes.combine(shape,Auxiliaries.getUnionShape(Auxiliaries.getMappedAABB(Auxiliaries.getRotatedAABB(side_aabb,
                  Direction.WEST, true), bb->bb.expand(0, railing_height_extension, 0))), IBooleanFunction.OR);
          if(shape.isEmpty()) shape = VoxelShapes.fullCube();
