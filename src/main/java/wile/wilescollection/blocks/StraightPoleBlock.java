...
public class StraightPoleBlock extends StandardBlocks.DirectedWaterLoggable
{
  private final StraightPoleBlock default_pole;

  public StraightPoleBlock(long config, BlockBehaviour.Properties builder, final AABB unrotatedAABB, @Nullable StraightPoleBlock defaultPole)
  ...
  public BlockState getStateForPlacement(BlockPlaceContext context)
  {
    ...
    if((config & StandardBlocks.CFG_FLIP_PLACEMENT_IF_SAME) != 0) {
      ...
      if(world.getBlockState(pos.relative(facing.getOpposite())).getBlock() instanceof StraightPoleBlock) {
  ...
  public InteractionResult use(BlockState state, Level world, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hit)
  {
    ...
    if(!(((BlockItem)(held_stack.getItem())).getBlock() instanceof StraightPoleBlock)) return InteractionResult.PASS;
