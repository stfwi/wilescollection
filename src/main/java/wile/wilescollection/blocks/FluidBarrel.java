...
public class FluidBarrel
{
  ...
  public static class FluidBarrelBlock extends StandardBlocks.DirectedWaterLoggable implements StandardBlocks.IStandardBlock, StandardBlocks.IBlockItemFactory
  {
    ...
    { return new FluidBarrel.FluidBarrelTileEntity(); }
  ...
  public static class FluidBarrelTileEntity extends TileEntity implements ICapabilityProvider, ITickableTileEntity
  {
    ...
    public boolean handlePlayerInteraction(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand)
    {
      if(world.isRemote()) return false;
      {
        ...
        if(transferred==null) {
          world.playSound(null, pos, SoundEvents.BLOCK_WOODEN_TRAPDOOR_OPEN, SoundCategory.BLOCKS, 0.2f, 0.02f);
      ...
      }
      {
        ...
        if((vol>0) && (cap>0)) {
          Overlay.show(player, Auxiliaries.localizable("block.wilescollection.fluid_barrel.status", new Object[]{
            Integer.toString(vol), Integer.toString(cap), name
          }));
        } else {
          Overlay.show(player, Auxiliaries.localizable("block.wilescollection.fluid_barrel.status.empty", new Object[]{
